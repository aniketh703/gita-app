    ...
    <ScrollView style={[styles.container, { backgroundColor: color.bg }]}>
      <View style={styles.header}>
        ...
        <View style={[styles.statsContainer, { borderColor: color.border }]}>
          ...
          <View style={styles.statItem}>
            <Text style={[styles.statNumber, { color: color.accent }]}>{getTotalVerses()}</Text>
            <Text style={[styles.statLabel, { color: color.secondary }]}>
      ...
      <View style={styles.content}>
        <Link href="/chapters" asChild>
          <TouchableOpacity style={[styles.button, { backgroundColor: color.accent }]}>
            <Text style={[styles.buttonText, { color: '#ffffff' }]}>{readLabel}</Text>
          </TouchableOpacity>
        </Link>

        <View style={[styles.infoBox, { backgroundColor: color.border, borderColor: color.border }]}>
          <Text style={[styles.infoTitle, { color: color.text }]}>
            {language === 'english'
              ? 'About this App'
              : 'इस ऐप के बारे में'}
          </Text>
          <Text style={[styles.infoText, { color: color.secondary }]}>
            {language === 'english'
              ? 'Explore the Bhagavad Gita in your preferred language with a clean, distraction-free reading experience. All content is available offline.'
              : 'भगवद्गीता को अपनी पसंदीदा भाषा में खोजें। सभी सामग्री ऑफ़लाइन उपलब्ध है।'}
          </Text>
        </View>
      </View>
    </ScrollView>
